  ...
  (let* ((disp (eat--t-term-display eat--t-term))
    ...
    (setq multi-col-char-indices
          (cl-loop for i from beg to (1- end)
                   for c = (aref str i)
                   when (and (> c 127) (/= (char-width c) 1))
                   collect (cons i (char-width c))))
                 ...
                 (let ((s-len (- e inserted-till))
                   ...
                   (cond
                    ((eq charset 'us-ascii)
                     ...
                     ;; is necessary.
                     nil)
                    ((eq charset 'dec-line-drawing)
                     (save-excursion
                       (goto-char start-pos)
                       (dotimes (_ s-len)
                         (when-let* ((r (gethash (char-after) eat--t-dec-line-drawing-chars)))
                           (delete-char 1)
                           (insert r))
                         (forward-char 1))))
                    (t (error "No clause matching `%S'" charset)))
                   ;; Add face.
                   (put-text-property start-pos (point) 'face face)
                   (put-text-property start-pos (point) 'font-lock-face face))
         ...
         ((and p1 (<= 30 p1 37))
          (setf (eat--t-face-fg face)
                (face-foreground
                 ...
                 nil t)))
         ((eql p1 38)
          (let* ((p_sub (pop params))
                 (p_sub1 (car-safe p_sub)))
            (cond
             ((eql p_sub1 2)
              (setf (eat--t-face-fg face)
    ...
    (save-excursion
      ...
      (while (< (point) end)
        (let* ((l-start (point))
               ...
               (act-end (min l-end end))
               (str (buffer-substring-no-properties l-start act-end))
               (trimmed-right (replace-regexp-in-string "[ \t]+$" "" str))
               (indent (if (string-match "^[ \t]+" str)
                           (length (match-string 0 str))
                         0))
               (content (if (string= trimmed-right "")
                            ""
                          (substring trimmed-right (min indent (length trimmed-right)))))
               (empty-p (string= content ""))
               (heading-p (and (< indent 4)
                               (string-match-p "^#+\\([ \t]\\|$\\)" content)
                               t))
               (bullet-p (and (string-match-p "^\\([•◦▪]\\|[*+-][ \t]\\)" content) t))
               (unicode-bullet-p (and (string-match-p "^[•◦▪]" content) t))
               (delim-p (and (string-match-p "^─+$" content) t))
               (colon-p (string-suffix-p ":" trimmed-right)))
          (push (list :start l-start :end act-end :str trimmed-right
                      :indent indent :content content :empty empty-p
                      :heading heading-p :bullet bullet-p
                      :unicode-bullet unicode-bullet-p :delim delim-p
                      :colon colon-p :code nil :wrapped nil)
                line-info)
          (goto-char (min end (1+ l-end))))))
    (setq line-info (nreverse line-info))
    ;; Identify code blocks in line-info
    (let ((n (length line-info))
          (i 0))
      (while (< i n)
        (let* ((ld (nth i line-info))
               (prev-ld (when (> i 0) (nth (1- i) line-info)))
               (start-trig (or (= i 0)
                               (and prev-ld
                                    (or (plist-get prev-ld :empty)
                                        (plist-get prev-ld :colon))))))
          (if (and start-trig
                   (not (plist-get ld :empty))
                   (not (plist-get ld :heading))
                   (not (plist-get ld :bullet))
                   (not (plist-get ld :delim))
                   (>= (plist-get ld :indent) 4))
              (let ((block (list i))
                    (j (1+ i))
                    (stop nil))
                (while (and (< j n) (not stop))
                  (let* ((next-ld (nth j line-info))
                         (next-next-ld (when (< (1+ j) n) (nth (1+ j) line-info))))
                    (cond
                     ((plist-get next-ld :empty)
                      (if (and next-next-ld
                               (not (plist-get next-next-ld :empty))
                               (not (plist-get next-next-ld :heading))
                               (not (plist-get next-next-ld :unicode-bullet))
                               (not (plist-get next-next-ld :delim))
                               (>= (plist-get next-next-ld :indent) 4))
                          (progn
                            (push j block)
                            (setq j (1+ j)))
                        (setq stop t)))
                     ((or (plist-get next-ld :heading)
                          (plist-get next-ld :unicode-bullet)
                          (plist-get next-ld :delim))
                      (setq stop t))
                     ((>= (plist-get next-ld :indent) 4)
                      (push j block)
                      (setq j (1+ j)))
                     ((< (plist-get next-ld :indent) 4)
                      (if (and next-next-ld
                               (or (>= (plist-get next-next-ld :indent) 4)
                                   (plist-get next-next-ld :delim)
                                   (plist-get next-next-ld :empty)))
                          (progn
                            (plist-put next-ld :wrapped t)
                            (push j block)
                            (setq j (1+ j)))
                        (setq stop t)))
                     (t (setq stop t)))))
                (dolist (idx block)
                  (plist-put (nth idx line-info) :code t))
                (setq i j))
            (setq i (1+ i))))))
    ;; Convert character regions to markdown with styling,
    ;; ignoring styles for positions in code blocks.
    (let ((pos beg)
      ...
      (while (< pos end)
        (while (and line-ptr
                    (<= (plist-get (car line-ptr) :end) pos)
                    (< (1+ pos) end))
          (setq line-ptr (cdr line-ptr)))
        (let* ((in-code-p (and line-ptr (plist-get (car line-ptr) :code)))
               (next-prop (next-property-change pos nil end))
          ...
          (if (= (char-after pos) ?\n)
              (progn
                ...
                (push "\n" output)
                (setq pos (1+ pos)))
            (let* ((face (unless in-code-p (get-text-property pos 'face)))
                   (font-lock-face (unless in-code-p (get-text-property pos 'font-lock-face)))
                   (combined-face (cond
                                   ((and face font-lock-face) (list face font-lock-face))
                                   (face face)
                                   (font-lock-face font-lock-face)
                                   (t nil)))
                   (next-state (if in-code-p '(nil nil nil) (eat--resolve-markdown-styles combined-face)))
      ...
      (let* ((raw-md (apply #'concat (nreverse output)))
             (raw-lines (split-string raw-md "\n"))
             (items nil)
             (idx 0)
             (n-lines (length line-info))
             (base-margin 999))
        (dotimes (k n-lines)
          (let ((ld-k (nth k line-info)))
            (when (and (not (plist-get ld-k :code))
                       (not (plist-get ld-k :empty)))
              (let ((ind (plist-get ld-k :indent)))
                (when (< ind base-margin)
                  (setq base-margin ind))))))
        (when (= base-margin 999) (setq base-margin 0))
        (while (< idx n-lines)
          (let ((ld (nth idx line-info))
                (raw-line (or (nth idx raw-lines) "")))
            (if (plist-get ld :code)
                ;; Collect contiguous code block lines
                (let ((code-block-lines nil)
                      (c-idx idx))
                  (while (and (< c-idx n-lines)
                              (plist-get (nth c-idx line-info) :code))
                    (let ((c-ld (nth c-idx line-info)))
                      (if (plist-get c-ld :wrapped)
                          (when code-block-lines
                            (let ((prev (car code-block-lines)))
                              (setcar code-block-lines
                                      (concat prev " " (plist-get c-ld :content)))))
                        (push (plist-get c-ld :str) code-block-lines)))
                    (setq c-idx (1+ c-idx)))
                  (setq code-block-lines (nreverse code-block-lines))
                  ;; Find min-indent
                  (let ((min-indent 999))
                    (dolist (s code-block-lines)
                      (when (not (string= (string-trim s) ""))
                        (let ((ind (if (string-match "^[ \t]+" s)
                                       (length (match-string 0 s))
                                     0)))
                          (when (< ind min-indent)
                            (setq min-indent ind)))))
                    (when (= min-indent 999) (setq min-indent 0))
                    (push (list :type 'code :text "```") items)
                    (dolist (s code-block-lines)
                      (if (string= (string-trim s) "")
                          (push (list :type 'code :text "") items)
                        (push (list :type 'code
                                    :text (if (>= (length s) min-indent)
                                              (substring s min-indent)
                                            s))
                              items)))
                    (push (list :type 'code :text "```") items))
                  (setq idx c-idx))
              ;; Normal non-code line
              (if (plist-get ld :empty)
                  (push (list :type 'empty :text "") items)
                (let* ((rel-indent (max 0 (- (plist-get ld :indent) base-margin)))
                       (trimmed (replace-regexp-in-string "[ \t]+\\([*`]*\\)$" "\\1" raw-line))
                       (content (if (string-match "^[ \t]+" trimmed)
                                    (substring trimmed (match-end 0))
                                  trimmed)))
                  (setq content (replace-regexp-in-string "^[•◦▪][ \t]+" "- " content))
                  (cond
                   ((plist-get ld :heading)
                    (push (list :type 'header :text content) items))
                   ((plist-get ld :delim)
                    (push (list :type 'delim :text content) items))
                   ((string-match-p "^\\(-\\|\\*\\|\\+\\|[0-9]+[\\.\\)]\\)[ \t]" content)
                    (let ((prefix (if (>= rel-indent 4) "  " "")))
                      (push (list :type 'list :text (concat prefix content)) items)))
                   (t
                    (let ((prefix (if (>= rel-indent 4) "  " "")))
                      (push (list :type 'prose :text (concat prefix content)) items))))))
              (setq idx (1+ idx)))))
        ...
        (let ((result nil))
          (dolist (item items)
            (let ((prev (car result)))
              (if (and prev
                       (eq (plist-get item :type) 'prose)
                       (memq (plist-get prev :type) '(prose list)))
                  (plist-put prev :text
                             (concat (plist-get prev :text)
                                     " "
                                     (replace-regexp-in-string "^[ \t]+" "" (plist-get item :text))))
                (push item result))))
  ...
  (dolist (window windows)
    (if (eq window 'buffer)
      ...
      (with-selected-window window
        ...
        (recenter
         (- (1- (eat--t-cur-y (eat--t-disp-cursor (eat--t-term-display eat-terminal))))
        ...
        (save-restriction
          ...
          (let ((inhibit-read-only t)
            ...
            (let ((beg (eat-term-beginning eat-terminal))
              ...
              ;; Only add properties if the region has grown or properties are missing at the end
              ;; Actually, just adding them to the *newly inserted* text at the end would be much faster.
              ;; Since eat-term appends to the end most of the time, we can track the last processed end.
              (unless (and (local-variable-p 'eat--last-processed-end)
                           (= eat--last-processed-end end))
                (let ((start-prop (if (and (local-variable-p 'eat--last-processed-end)
                                           (<= beg eat--last-processed-end)
                                           (<= eat--last-processed-end end))
                                      eat--last-processed-end
                                    beg)))
                  (when (< start-prop end)
                    (add-text-properties
                     start-prop end
                     `( read-only t field eat-terminal
                        ,@(when eat--line-mode
                            '(front-sticky t rear-nonsticky t))))
                    (setq-local eat--last-processed-end end)))))))
